-- ============================================================
-- ZENITH LIFE OS — W04 MIGRATIONS (FIXED)
-- يُنشئ public.users أولاً ثم يضيف كل جداول W04
-- ============================================================

-- STEP 0: أنشئ public.users لو مش موجود (مرتبط بـ auth.users)
...
-- trigger: ينشئ صف تلقائياً عند تسجيل مستخدم جديد
...
-- 0401: users extend w04 (أضف الأعمدة الجديدة)
...
-- 0402: password_reset_tokens — TTL 60 دقيقة
...
-- تأكيد النجاح
SELECT 
  'W04 migrations applied ✅' AS status,
  count(*) AS tables_created
FROM information_schema.tables 
WHERE table_schema = 'public' 
  AND table_name IN (
    'users','oauth_state_tokens','password_reset_tokens',
    'magic_link_tokens','email_verification_tokens',
    'auth_lockouts','webauthn_credentials','onboarding_state','captcha_attempts'
  );
